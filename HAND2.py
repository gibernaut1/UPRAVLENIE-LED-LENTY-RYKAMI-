...
portNo = "COM5"                         # указываем последовательный порт, к которому подключена Arduino
...
while True:
    ...
    if results.multi_hand_landmarks:                            # если обнаружили точки руки
        for handLms in results.multi_hand_landmarks:            # получаем координаты каждой точки
            ...
            # при помощи инструмента рисования проводим линии qqмежду точками
            ...
            for id, point in enumerate(handLms.landmark):
                ...
                if id==16:
                    cv2.circle(img, (width,height), 15, ( 0, 255, 0),cv2.FILLED)
                if id==20:
            ...
            def sel():
                      ...
                      driver.get("https://skd.sp-oberig.com.ua/json/mapi/camera.php?pic=1&object_id=21&camera_id=3")
                      time.sleep(4)
                      while e<10:
                          driver.save_screenshot('d:\\image5\\' + str(e) + '.jpg')
                          driver.refresh()
                          e=e+1
